...
object testDistribution {
  ...
  private def testGaussian(): Unit ={
    val ef_gaussian = EF_Gaussian(VariableFactory.newGaussianVariable(""), 0, 1)
    val gaussian = Gaussian(VariableFactory.newGaussianVariable("s"))
  ...
  private def testMultinomial(): Unit ={
    ...
    val ef_multinomial = EF_Multinomial(VariableFactory.newMultinomialVariable("", 3), Vector(0.5, 0.3, 0.2))
    val multinomial = Multinomial(VariableFactory.newMultinomialVariable("", 3), Vector(0.5, 0.3, 0.2))
