...
object testDistribution {
  ...
  private def testGaussian(): Unit ={
    val ef_gaussian = EF_Gaussian(MainVariablesFactory.newGaussianLV(""), 0, 1)
    val gaussian = Gaussian(MainVariablesFactory.newGaussianLV("s"))
  ...
  private def testMultinomial(): Unit ={
    ...
    val ef_multinomial = EF_Multinomial(MainVariablesFactory.newMultinomialLV("", 3), Vector(0.5, 0.3, 0.2))
    val multinomial = Multinomial(MainVariablesFactory.newMultinomialLV("", 3), Vector(0.5, 0.3, 0.2))
