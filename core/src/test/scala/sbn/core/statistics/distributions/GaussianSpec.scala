...
class GaussianSpec extends CustomSpec{
  ...
  "Gaussian.label" should "return 'Gaussian'" in {
    assert(Gaussian(ModelVariablesFactory.newGaussianLV("gaussian_noBounds")).label == "Gaussian")
  }

  "Gaussian.numberOfParameters" should "be == 2" in {
    assert(Gaussian(ModelVariablesFactory.newGaussianLV("gaussian_noBounds")).numberOfParameters == 2)
  }
