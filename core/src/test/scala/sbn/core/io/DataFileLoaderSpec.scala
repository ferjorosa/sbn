...
class DataFileLoaderSpec extends CustomSpec{
  ...
  "DataFileLoader" should "return a Failure if the file's extension is not supported" in {
    val exception1 = shouldFail(DataFileLoader.loadImmutableDataSet("datasets/test/sbn.examples.core/io/DataFileLoaderSpec/fileExtensionNotSupported"))
    val exception2 = shouldFail(DataFileLoader.loadImmutableDataSet("datasets/test/sbn.examples.core/io/DataFileLoaderSpec/fileExtensionNotSupported.lol"))
  ...
  it should "return a Failure if the provided path is a directory" in {
    val exception = shouldFail(DataFileLoader.loadImmutableDataSet("datasets/test/sbn.examples.core/io/DataFileLoaderSpec"))
